...
\begin{itemize}
    ...
    \item \textbf{Gestión de estado y ciclo de vida:} seguimiento de los
    manifiestos desplegados, su proveedor y su versión, con almacén embebido o
    backend remoto para entornos multi-usuario, como base para gobernar el ciclo
    de vida de la infraestructura desde la propia herramienta.
\end{itemize}
...
\begin{itemize}
    ...
    de seguridad cloud, como \textbf{Prowler}~\cite{prowler}, que evalúen la
    infraestructura ya desplegada frente a marcos de cumplimiento como CIS o NIST.
\end{itemize}
...
En concreto, el subconjunto abordado en este trabajo comprende:

\begin{itemize}
    \item \textbf{Dos proveedores cloud:} AWS y Microsoft Azure.
    \item \textbf{Traducción y despliegue de extremo a extremo:} un Manifest en YAML,
    agnóstico al proveedor, que se traduce de forma determinista a plantillas de OpenTofu
    y se despliega sobre el proveedor elegido.
    \item \textbf{Tipos de recurso:} cómputo, base de datos relacional gestionada y red
    autogenerada.
    \item \textbf{Estimación comparativa de costes} entre los proveedores soportados.
    \item \textbf{Auditoría de seguridad} a nivel de manifiesto, mediante la detección de
    anti-patrones.
    \item \textbf{Interfaz de línea de comandos} como punto de entrada a todas estas
    capacidades.
\end{itemize}

El resto de la visión completa queda fuera de esta versión y se recoge como líneas de
evolución futura al final del capítulo.

\section{Requisitos funcionales}

A partir del alcance definido en la sección anterior, los siguientes requisitos
funcionales definen el comportamiento del sistema en su versión inicial. Se presentan
...
\begin{itemize}
    ...
    \item Análisis de seguridad avanzado: análisis estático de la infraestructura
    generada —permisos excesivos, puertos expuestos— y auditoría continua del
    cumplimiento, más allá de la detección de anti-patrones a nivel de manifiesto.
