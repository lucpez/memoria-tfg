...
De forma complementaria, el directorio de ejemplos del proyecto incluye \textbf{manifiestos
...
en ambos casos. Este resultado constituye la evidencia central de la interoperabilidad que persigue
el trabajo.

\Lucas{Insertar una o dos capturas como evidencia del despliegue: la aplicación de inventario accesible en el navegador y, opcionalmente, el resumen de recursos creados que imprime \texttt{deploy}.}
...
La suite de tests automatizados alcanzó una cobertura del 86\,\%, por encima del umbral mínimo del
...
Conviene, no obstante, enmarcar estos resultados en el carácter de prueba de concepto del trabajo,
del que se derivan varias limitaciones. La herramienta soporta dos proveedores y un conjunto acotado
de tipos de recurso, suficientes para demostrar la viabilidad, pero lejos de la cobertura de un
producto. CloudPorter actúa además en la fase de diseño y despliegue, no migra infraestructura ya
desplegada en producción. El análisis de seguridad opera a nivel de manifiesto y no sustituye al
análisis estático del HCL generado que realizan herramientas especializadas. Por último, quedan
fuera del alcance los servicios con lock-in a nivel de código, como el almacenamiento de objetos.

Estas limitaciones no contradicen los resultados, sino que delimitan su alcance: la evaluación
demuestra que el enfoque funciona dentro del subconjunto definido, y señala con claridad las
direcciones en las que el trabajo puede crecer, que se retoman en el capítulo~\ref{cap:conclusiones}.
