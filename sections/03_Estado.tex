...
elaborar un catálogo exhaustivo, sino evidenciar que las soluciones actuales atienden
las distintas dimensiones del lock-in de forma aislada: cada categoría resuelve una
parte del problema, pero ninguna lo aborda de manera integrada. Para ello, las
herramientas se han agrupado en cinco categorías que se analizan de forma independiente:
infraestructura como código, abstracción multi-cloud, estimación de costes, plataformas
como servicio y herramientas con componente de inteligencia artificial. Sus capacidades
se reúnen finalmente en una comparativa (tabla~\ref{tab:comparativa}) con la que enmarcar
la contribución de CloudPorter dentro del estado del arte.
...
Para cada dimensión, «Sí» indica que la herramienta aborda esa forma de lock-in, 
«Parcial» que la aborda parcialmente y «No» que no la aborda.
...
Crossplane              & Sí      & Parcial & No      \\
Score                   & No      & Parcial & No      \\
Infracost               & No      & No      & Parcial \\
CloudHealth             & No      & No      & Parcial \\
Railway / Render / Fly  & Sí      & Sí      & No      \\
...
dimensiones del lock-in identificadas en el capítulo anterior:

\begin{itemize}
    \item \textbf{Infraestructura como código multi-proveedor:} reduce parcialmente
    el lock-in de infraestructura, pero traslada al usuario la responsabilidad de
    conocer los recursos específicos de cada cloud, sin abordar el lock-in de
    conocimiento ni el económico.

    \item \textbf{Abstracción multi-cloud:} ofrece una capa de abstracción más
    genuina, pero ninguna herramienta es autosuficiente: cada una deja fuera el
    aprovisionamiento específico de cada proveedor —Crossplane lo delega en un plano
    de control que el usuario debe operar, Score en una plataforma externa y
    MultiCloudJ en el propio código de la aplicación— y ninguna ofrece visibilidad
    económica.

    \item \textbf{Plataformas como servicio:} abstraen completamente la
    infraestructura, a costa de generar una nueva forma de dependencia con el propio
    PaaS.

    \item \textbf{Gestión del gasto cloud:} permite estimar costes antes del
    despliegue, pero opera sobre infraestructura ya definida para un proveedor
    concreto, sin posibilidad de comparar entre proveedores.

    \item \textbf{Componente de inteligencia artificial:} delega decisiones críticas
    a modelos de lenguaje sin garantías deterministas.
\end{itemize}
