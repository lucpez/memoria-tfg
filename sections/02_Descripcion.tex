...
Más allá de estas dimensiones, conviene distinguir el \textbf{nivel} en el que esta
dependencia técnica se manifiesta. El lock-in de infraestructura descrito reside en los
recursos que se aprovisionan, pero existe una forma más arraigada, el \textbf{lock-in a
nivel de código}: cuando la propia aplicación utiliza de forma directa los servicios
gestionados de un proveedor, la dependencia abandona la definición de la infraestructura
y queda grabada en el código fuente. Migrar a otra nube ya no se limita entonces a las
tres dimensiones anteriores —redescribir los recursos, reaprender los servicios y
reestimar el coste—, sino que las arrastra hasta el propio código, que debe reescribirse
o refactorizarse para funcionar con los servicios del nuevo proveedor. Es el mismo
problema una capa por debajo, donde resulta bastante más difícil de revertir.

A todo ello se añade la \textbf{complejidad de seguridad} inherente
...
servicios— afecten directamente a su operativa. Esa debilidad es la otra cara de la
falta de alternativas: quien puede trasladar de forma creíble su infraestructura a
otro proveedor conserva la palanca para negociar descuentos o mejores condiciones,
mientras que quien depende de uno solo la pierde.
...
\begin{itemize}
    ...
    \item \textbf{Resolución del lock-in a nivel de código:} abstraer la dependencia que el
    código de la aplicación contrae con servicios concretos de un proveedor —como
    \textbf{AWS S3} o \textbf{Azure Blob Storage}— exigiría una interfaz agnóstica a casi
    todos los servicios de cada nube, un problema de complejidad muy superior que excede
    este trabajo.
